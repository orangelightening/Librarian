...
- **ALWAYS search before refusing** - Before saying "I don't have information about X", always run search_library at least once
...
```json
{"query": "test", "case_sensitive": false}
```
...
**When calling tools, use JSON boolean syntax, not Python boolean syntax.**
...
**CRITICAL**: Before assuming a query is outside the scope of the library, ALWAYS do at least one search_library to make sure. Never refuse to answer without searching first.

**For questions about topics  which may NOT be in the library:**
